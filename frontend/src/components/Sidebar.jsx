...
const Sidebar = ({ activePage, setActivePage }) => {
  ...
  const menuItems = [
    ...
    { id: "inventory", icon: checklists, label: "Inventory", path: "/inventory" },
    { id: "faculty", icon: groupIcon, label: "Faculty", path: "/faculty" },
  ];

  return (
    <div className="bg-orange-600 text-white flex sm:flex-col sm:w-45 h-16 sm:h-screen w-full fixed sm:relative bottom-0 sm:top-0 z-50" style={{ boxShadow: "8px 8px 20px rgba(0,0,0,0.4)" }}>
      {/* Menu */}
        ...
        {menuItems.map((item) => (
          <button
            ...
            className={`flex flex-col items-center justify-center sm:w-full sm:p-5 transition
              ${
                activePage === item.id
                  ? "bg-orange-500 sm:border-l-4 sm:border-orange-300 text-white"
                  : "hover:bg-orange-500"
              }`}
          >
            <img
