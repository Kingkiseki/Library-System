...
const QRPopup = ({ qrData, onClose }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (parsedData.type === "student") {
        ...
        console.log("📚 Detected book QR code");
        console.log("Book QR data:", parsedData);
        
        // Store the book QR data for borrowing/returning
        ...
        if (buffer.trim() !== "" && buffer.length > 5) {
          ...
          try {
            ...
            if (parsedData.type === "book") {
              console.log("📚 Book QR Scanner - Detected book QR code");
              setBookQR(JSON.stringify(parsedData));
  ...
  const confirmReturn = async () => {
    ...
    borrowRecord = studentProfile.borrowedBooks.find(b => {
      ...
      // Check for direct ObjectId match (most reliable for new format)
      if (bookId === scannedBookId) {
        console.log("✅ Found exact ObjectId match");
        return true;
      }
    ...
    try {
      ...
      alert(`✅ Book "${borrowRecord.book?.name}" returned successfully!`);
                    ...
                    <div className="bg-green-50 p-4 rounded-lg mb-4">
                      ...
                      <div className="text-sm text-green-600">
                        {(() => {
                          ...
                          return <p>Book ID: {returnBookQR}</p>;
                        })()}
                      </div>
                    </div>
                    <div className="flex gap-4 justify-center">
