...
const TotalBooksReport = () => {
  ...
  // ✅ Fetch backend stats from inventory endpoint
  useEffect(() => {
    api.get("/books/stats").then((res) => setStats(res.data)).catch(err => {
      console.error("Error fetching book stats:", err);
      // Keep default stats (all zeros) which will trigger fallback data
    });
  }, []);
