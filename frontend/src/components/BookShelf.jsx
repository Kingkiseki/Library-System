...
const BookShelf = () => {
  ...
  const handleShowQRCode = (item) => {
    // Get title and author from the item with proper fallbacks
    const title = item["Book Title"] || item.booktitle || item.title || item.name || "Unknown Item";
    const author = item.Author || item.author || "Unknown Author";
    
    // Ensure we have valid, non-empty strings
    const safeTitle = (title && title.toString().trim()) || "Unknown Item";
    const safeAuthor = (author && author.toString().trim()) || "Unknown Author";
 ...
 const handleAddBook = async () => {
  ...
  if (activeCategory === "masterlist") {
    ...
    itemToAdd = {
      "No": newBook["No"] || Math.floor(Math.random() * 10000),
      "Collection Type": newBook["Collection Type"] || "Physical",
      "Gen Ed Prof Ed": newBook["Gen Ed Prof Ed"] || newBook["Gen.Ed./Prof.Ed."] || "General Education",
      "Course Name": newBook["Course Name"] || "General Course",
      "Book Title": newBook["Book Title"] || newBook.name || "Untitled",
      "Author": newBook.author || newBook["Author"] || "Unknown Author",
      "Publication Year": parseInt(newBook["Publication Year"]) || new Date().getFullYear(),
      "No of Book Copies": parseInt(newBook["No of Book Copies"]) || parseInt(newBook["No. of Book Copies"]) || 1
    };
    ...
    itemToAdd = {
      "No": newBook["No"] || 1,
      "Collection Type": newBook["Collection Type"] || "Physical",
      "Classification": newBook["Classification"] || "General",
      "Course Name": newBook["Course Name"] || "General Course",
      "Book Title": newBook["Book Title"] || newBook.name || "Untitled",
      "Author": newBook.author || newBook["Author"] || "Unknown Author",
      "Publication Year": parseInt(newBook["Publication Year"]) || new Date().getFullYear(),
      "Volumes": parseInt(newBook["Volumes"]) || 1
    };
  ...
  return (
    <div className="p-4 sm:p-6 w-full bg-gray-100 min-h-screen">
      ...
      {/* Excel-like editing info for all categories */}
      <div className="mb-4 bg-orange-50 border border-orange-200 rounded-lg p-3">
        <div className="flex items-center gap-2 text-blue-800">
          <span className="text-sm">📊</span>
          <span className="text-sm font-medium">Excel-like Editing Mode - {getCurrentConfig().label}</span>
        </div>
        <p className="text-xs text-blue-600 mt-1">
          Click on any cell to edit directly. Press Enter to save or Escape to cancel.
        </p>
      </div>
          ...
          <div className="mx-4 my-4 space-y-4">
            ...
            {summaryData.professionalEducation.length > 0 && (
              <div className="bg-white border rounded-lg shadow-sm">
                <div className="bg-blue-500 px-4 py-2 border-b">
                  <h3 className="text-sm font-bold text-white">Professional Education</h3>
                </div>
                <div className="bg-gray-50 px-4 py-1 border-b">
                  <h4 className="text-xs font-semibold text-gray-700">SUMMARY OF REPORT</h4>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                    <thead>
            ...
            {summaryData.generalEducation.length > 0 && (
              <div className="bg-white border rounded-lg shadow-sm">
                <div className="bg-green-500 px-4 py-2 border-b">
                  <h3 className="text-sm font-bold text-white">General Education</h3>
                </div>
                <div className="bg-gray-50 px-4 py-1 border-b">
                  <h4 className="text-xs font-semibold text-gray-700">SUMMARY OF REPORT</h4>
                </div>
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md mx-4">
            <h2 className="text-lg font-bold mb-4 text-center text-gray-800">
              QR Code for "{selectedBook["Book Title"] || selectedBook.name || "Unknown Book"}"
            </h2>
            <div className="flex flex-col items-center">
              <div ref={bookQRRef}>
                <QRCodeCanvas
                  value={selectedBook.qrData ? JSON.stringify(selectedBook.qrData) : JSON.stringify({
                    ...
                    title: selectedBook["Book Title"] || selectedBook.booktitle || selectedBook.title || selectedBook.name || "Unknown Item",
              ...
              <div className="text-xs text-gray-500 mt-2">
                <p><strong>Book:</strong> {selectedBook["Book Title"] || selectedBook.name || "Unknown Title"}</p>
                <p><strong>Author:</strong> {selectedBook.Author || selectedBook.author || "Unknown Author"}</p>
              </div>
            </div>
